...
export function useExternalWallet() {
  ...
  // Keep wcProviderRef current so setInterval callbacks can read the latest
  // provider without calling a hook inside the timer (Rules of Hooks violation).
  useEffect(() => { wcProviderRef.current = wcProvider; }, [wcProvider]);
  ...
  const openModal = useCallback(async (): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      ...
      pollRef.current = setInterval(() => {
        ticks++;
        // Read session state directly from the provider object ref — no hook
        // call inside the timer, which would violate Rules of Hooks.
        // wcProviderRef.current is kept in sync by a useEffect above.
        const prov        = wcProviderRef.current;
        const sessionAddr = prov ? extractAddressFromSession(prov) : null;
        const sessionCId  = prov ? extractChainIdFromSession(prov) : null;
        const connected   = !!sessionAddr;

        console.log(`[WC] poll #${ticks} → connected:${connected} addr:${sessionAddr ?? 'none'} chain:${sessionCId ?? 'none'} provider:${prov ? 'yes' : 'no'}`);

        if (connected && sessionAddr) {
          console.log('[WC] Session confirmed by poll ✔ addr:', sessionAddr);
          stopPoll();
  ...
  // Must match USDT_CONTRACTS in bdag-deposit and bdag-withdraw Edge Functions.
  // Base (8453): Circle's native USDC — no official Tether USDT exists on Base mainnet.
  // Source: https://developers.circle.com/stablecoins/docs/usdc-on-main-networks
  const USDT_CONTRACTS: Record<number, string> = {
    1:    '0xdac17f958d2ee523a2206206994597c13d831ec7', // Ethereum mainnet — Tether USDT
    8453: '0x833589fcd6edb6e08f4c7c32d4f71b54bda02913', // Base mainnet — Circle USDC (canonical stablecoin on Base)
  };
  ...
  const sendToTreasury = useCallback(async (
    ...
    if (isUSDT) {
      // Resolve contract from targetNetwork first — avoids a React state-update
      // race where chainId still reflects the pre-switch chain when sendToTreasury
      // is called immediately after switchNetwork() in the same event handler.
      const activeChainId =
        NETWORKS[targetNetwork ?? '']?.chainId ?? chainId ?? 1;
