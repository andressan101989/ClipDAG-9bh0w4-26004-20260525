...
export function MarketplaceShippingQuoteCard({productId,quantity,countryCode,regionCode,onChange}:{productId:string;quantity:number;countryCode?:string|null;regionCode?:string|null;onChange?:(state:MarketplaceShippingQuoteState)=>void}){
 ...
 return <View style={styles.card} accessibilityLabel="Disponibilidad de envío">
  ...
  {state.status==='idle'||state.status==='error'?<><Text style={state.status==='error'?styles.error:styles.text}>{state.message}</Text>{state.status==='error'?<Pressable onPress={()=>void load()} style={styles.retry}><Text style={styles.retryText}>Reintentar</Text></Pressable>:null}</>:null}
