...
alter table public.marketplace_shipping_profile_regions
 add column archived_at timestamptz,
 add constraint marketplace_shipping_archived_rule_paused
 check(archived_at is null or status='paused');
...
begin
 ...
 for r in select*from jsonb_array_elements(p_regions)loop
  ...
  if nullif(btrim(coalesce(r->>'id','')),'')is null then
   select x.id into rule_id from public.marketplace_shipping_profile_regions x
    where x.profile_id=result and x.country_code=country and x.region_code is not distinct from region
     and x.archived_at is not null for update;
   if rule_id is null then
    ...
    update public.marketplace_shipping_profile_regions set shipping_price=price,free_shipping_threshold=threshold,
     transit_days_min=days_min,transit_days_max=days_max,status='active',archived_at=null,updated_at=now()
    where id=rule_id and profile_id=result;
   end if;
  else
   ...
   rule_id:=(r->>'id')::uuid;
   if not exists(select 1 from public.marketplace_shipping_profile_regions x
    where x.id=rule_id and x.profile_id=result and x.archived_at is null)then
 ...
 update public.marketplace_shipping_profile_regions x set status='paused',archived_at=coalesce(x.archived_at,now()),updated_at=now()
 where x.profile_id=result and not(x.id=any(kept_ids))and exists(
  select 1 from public.marketplace_order_shipping_snapshots s where s.matched_rule_id=x.id)
  and(x.status<>'paused'or x.archived_at is null);
 delete from public.marketplace_shipping_profile_regions x where x.profile_id=result and not(x.id=any(kept_ids))
  and not exists(select 1 from public.marketplace_order_shipping_snapshots s where s.matched_rule_id=x.id);
 update public.marketplace_shipping_profiles p set configuration_status=case when exists(
  select 1 from public.marketplace_shipping_profile_regions x
  where x.profile_id=result and x.status='active'and x.archived_at is null
...
language sql stable security definer set search_path=public as $$select coalesce(jsonb_agg(jsonb_build_object(
 ...
 'regions',coalesce((select jsonb_agg(jsonb_build_object('id',r.id,'country_code',r.country_code,
  ...
  from public.marketplace_shipping_profile_regions r where r.profile_id=p.id and r.archived_at is null),'[]'::jsonb))
 order by p.created_at,p.id),'[]'::jsonb)from public.marketplace_shipping_profiles p
where p.store_id=p_store_id and p.seller_id=auth.uid()$$;
