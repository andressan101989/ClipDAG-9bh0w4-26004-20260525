-- R1C-F1A: reconcile buyer and Admin Marketplace settlement release actors without changing movement authority.
begin;
...
with e as(
  ...
  select l.*,s.order_id,s.buyer_id,s.seller_id,s.release_actor_id,s.release_actor_role,
  ...
  join marketplace_order_settlements s on s.id=l.settlement_id
...
select jsonb_build_object(
  ...
  'released_order_not_delivered',(select count(*) from marketplace_payment_allocations a left join marketplace_order_settlements s on s.allocation_id=a.id join marketplace_orders o on o.id=a.order_id where a.status='released' and(s.id is null or(s.release_actor_role='buyer'and o.status is distinct from 'delivered')or(s.release_actor_role='admin'and o.status not in('shipped','delivered')))),
  'released_shipment_not_delivered',(select count(*) from marketplace_payment_allocations a left join marketplace_order_settlements s on s.allocation_id=a.id left join marketplace_order_shipments sh on sh.order_id=a.order_id where a.status='released' and(s.id is null or(s.release_actor_role='buyer'and(sh.id is null or sh.status is distinct from 'delivered' or sh.delivered_at is null))or(s.release_actor_role='admin'and(sh.id is null or sh.status not in('shipped','delivered'))))),
  'delivery_timestamp_mismatch',(select count(*) from marketplace_order_settlements s join marketplace_orders o on o.id=s.order_id left join marketplace_order_shipments sh on sh.order_id=o.id where s.release_actor_role='buyer'and(sh.id is null or o.delivered_at is null or sh.delivered_at is null or o.delivered_at is distinct from sh.delivered_at or s.confirmed_at is distinct from o.delivered_at)),
  ...
  'transaction_reference_mismatch',(select count(*) from lt where amount>0 and tx is not null and(reference_type is distinct from'marketplace_order'or reference_id is distinct from order_id::text or initiated_by is distinct from case release_actor_role when'buyer'then buyer_id when'admin'then release_actor_id end)),
...
comment on function public.reconcile_marketplace_settlements() is
  'Read-only Marketplace settlement integrity counters with strict buyer-delivery and canonical Admin-release actor semantics.';
