...
alter table public.messages
  add column audio_waveform smallint[];

alter table public.messages
  add constraint messages_voice_contract_check check (
    (
      message_type = 'voice'
      and media_type = 'voice'
      and consumption_policy = 'standard'
      and media_asset_id is not null
      and media_url is null
      and audio_duration_ms between 1 and 3600000
      and array_length(audio_waveform, 1) = 48
      and array_position(audio_waveform, null) is null
...
begin
  ...
  if new.media_type not in ('text', 'image', 'video', 'premium_dm', 'one_time_image', 'voice') then
  ...
  if new.media_type <> 'voice' and (new.audio_duration_ms is not null or new.audio_waveform is not null) then
    raise exception 'chat_audio_metadata_not_allowed';
  end if;
  ...
  if new.media_asset_id is not null then
    ...
    if new.media_type = 'voice' then
      if v_asset.media_kind <> 'audio' or v_asset.purpose <> 'voice_note' then
        raise exception 'chat_voice_asset_invalid';
      end if;
    elsif new.media_type in ('image', 'one_time_image') then
      if v_asset.media_kind <> 'image' or v_asset.purpose <> 'chat_image' then
        raise exception 'chat_media_asset_invalid';
      end if;
    else
  ...
  new.message_type := coalesce(new.message_type, case
    ...
    when new.media_type = 'voice' then 'voice'
    else 'text'
  end);
...
drop function public.chat_send_message(uuid, uuid, text, text, text, uuid, uuid);
...
create function public.chat_send_message(
  ...
  p_audio_waveform smallint[] default null
)
...
begin
  ...
  if p_message_type not in ('text', 'image', 'video', 'one_time_image', 'voice') then
    raise exception 'chat_message_type_invalid';
  end if;
  ...
  if p_message_type = 'voice' and (
    p_media_asset_id is null or p_media_url is not null
    or p_audio_duration_ms is null or p_audio_duration_ms not between 1 and 3600000
  ...
  if p_message_type <> 'voice' and (p_audio_duration_ms is not null or p_audio_waveform is not null) then
    raise exception 'chat_audio_metadata_not_allowed';
  end if;
  ...
  if v_existing.id is not null then
    if v_existing.conversation_id <> p_conversation_id
      ...
      or v_existing.audio_waveform is distinct from p_audio_waveform then
  ...
  if p_media_asset_id is not null then
    ...
    if v_asset.id is null or v_asset.owner_id <> v_actor or v_asset.status <> 'ready'
      or v_asset.visibility <> 'private' or v_asset.provider <> 'r2'
      or v_asset.public_url is not null then raise exception 'chat_media_asset_invalid'; end if;
    if p_message_type = 'voice' then
      if v_asset.media_kind <> 'audio' or v_asset.purpose <> 'voice_note' then
        raise exception 'chat_voice_asset_invalid';
      end if;
    elsif p_message_type in ('image', 'one_time_image') then
      if v_asset.media_kind <> 'image' or v_asset.purpose <> 'chat_image' then
        raise exception 'chat_media_asset_invalid';
      end if;
    else
    ...
    if p_media_url is not null then raise exception 'chat_private_media_contract_invalid'; end if;
  ...
  insert into public.messages (
    ...
    reply_to_message_id, audio_duration_ms, audio_waveform, read
  ) values (
    ...
    p_reply_to_message_id, p_audio_duration_ms, p_audio_waveform, false
...
exception
  when unique_violation then
    ...
    if v_result.conversation_id <> p_conversation_id
      ...
      or v_result.audio_waveform is distinct from p_audio_waveform then
      raise exception 'chat_idempotency_conflict';
    end if;
    return v_result;
end;
...
revoke all on function public.chat_send_message(uuid, uuid, text, text, text, uuid, uuid, integer, smallint[])
  from public, anon;
grant execute on function public.chat_send_message(uuid, uuid, text, text, text, uuid, uuid, integer, smallint[])
...
begin
  ...
  where id = p_asset_id and status = 'ready' and visibility = 'private'
    and provider = 'r2' and public_url is null;
  if v_asset.id is null then raise exception 'chat_media_unavailable'; end if;

  if v_message.message_type = 'voice' then
    if v_message.consumption_policy <> 'standard' or v_asset.media_kind <> 'audio'
      or v_asset.purpose <> 'voice_note' then raise exception 'chat_media_policy_invalid'; end if;
  elsif v_asset.media_kind <> 'image' or v_asset.purpose <> 'chat_image' then
    raise exception 'chat_media_unavailable';
...
returns table (
  ...
  consumption_policy text, audio_duration_ms integer, audio_waveform smallint[], read boolean,
...
as $$
  select m.id, m.conversation_id, m.client_message_id, m.sender_id,
    ...
    m.consumption_policy, m.audio_duration_ms, m.audio_waveform, m.read, m.deleted_at, m.created_at,
