-- R2B-4 F1: route every shipped return to seller receipt confirmation.
-- The shipment state is authoritative even for immutable legacy rows that
-- predate seller-issued labels. This migration is read-model only and never
-- moves money or changes return/shipment data.
begin;
    ...
    select o.created_at,o.id,jsonb_build_object(
      ...
      'active_return_request',(select jsonb_build_object(
        ...
        'attention_reason',case
          ...
          when rr.status='approved'and rs.id is null then'label_pending'
          else null end
...
comment on function public.fetch_my_marketplace_sales(text,integer,timestamptz,uuid) is
  'Seller order read model. Shipped returns always require receipt confirmation, including immutable legacy shipments without labels.';
comment on function public.fetch_my_marketplace_returns(integer,timestamptz,uuid) is
  'Seller return attention read model. Shipment state outranks label presence so shipped legacy returns route to receipt confirmation.';
