...
create or replace function private.live_battle_series_try_lock_scope(
  p_series_id uuid,
...
begin
  if p_challenger_user_id is null or p_opponent_user_id is null or
     p_challenger_user_id = p_opponent_user_id then
  ...
  where actor.id in (p_challenger_user_id, p_opponent_user_id)
  order by actor.id
  for update nowait;
  ...
  where session.id in (p_challenger_session_id, p_opponent_session_id)
  order by session.id
  for update nowait;
...
alter function private.live_battle_series_try_lock_scope(uuid, uuid, uuid, uuid, uuid)
  owner to postgres;
revoke all on function private.live_battle_series_try_lock_scope(uuid, uuid, uuid, uuid, uuid)
...
declare
  ...
  v_now timestamptz;
  v_lock_deadline timestamptz := pg_catalog.clock_timestamp() + interval '750 milliseconds';
...
begin
  ...
  while v_attempts < v_max_attempts loop
    ...
    begin
      v_latest := private.live_battle_series_try_lock_scope(
        p_series_id,
        v_series_snapshot.challenger_user_id,
        v_series_snapshot.opponent_user_id,
  ...
  v_remaining := v_lock_deadline - v_now;
  if v_remaining <= interval '0 milliseconds' then
    raise exception using
      errcode = '55P03', message = 'live_battle_series_leave_busy';
  end if;
