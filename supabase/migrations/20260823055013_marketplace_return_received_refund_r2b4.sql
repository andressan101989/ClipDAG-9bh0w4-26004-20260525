-- R2B-4: seller-confirmed physical return receipt and atomic buyer refund.
-- Reuses the funded return escrow and immutable return-refund authority.
...
alter table public.marketplace_return_shipments
  ...
  add column seller_receipt_note text,
  add column seller_receipt_idempotency_key uuid,
  add column seller_receipt_fingerprint text;

create unique index marketplace_return_shipments_seller_receipt_idempotency_unique
  on public.marketplace_return_shipments(seller_id,seller_receipt_idempotency_key)
  where seller_receipt_idempotency_key is not null;

alter table public.marketplace_return_shipments
  add constraint marketplace_return_shipments_seller_receipt_note_check check(
    seller_receipt_note is null or(
      seller_receipt_note=regexp_replace(
        seller_receipt_note,'^[[:space:]]+|[[:space:]]+$','','g'
      )
      and char_length(seller_receipt_note) between 1 and 500
      and seller_receipt_note!~*'<[[:space:]]*/?[[:alpha:]][^>]*>'
    )
  ),
  add constraint marketplace_return_shipments_seller_receipt_fingerprint_check check(
    seller_receipt_fingerprint is null or(
      char_length(seller_receipt_fingerprint)=64
      and seller_receipt_fingerprint~'^[0-9a-f]{64}$'
    )
  );

alter table public.marketplace_return_shipments
  drop constraint marketplace_return_shipments_status_check;
alter table public.marketplace_return_shipments
  add constraint marketplace_return_shipments_status_check
  check(status in('awaiting_buyer_shipment','shipped','received'));
...
begin
  ...
  if old.status='shipped' then
    if coalesce(current_setting('app.marketplace_return_receipt',true),'off')<>'on'
      or new.status<>'received'
      or(new.recipient_name,new.line1,new.line2,new.city,new.region,new.postal_code,
        new.country,new.phone,new.seller_instructions,new.carrier_name,new.service_level,
        new.tracking_number,new.tracking_url,new.buyer_note,new.instructions_provided_at,
        new.shipped_at,new.seller_instruction_idempotency_key,
        new.seller_instruction_fingerprint,new.buyer_shipping_idempotency_key,
        new.buyer_shipping_fingerprint,new.destination_fingerprint,
        new.shipped_destination_fingerprint,new.return_label_asset_id,new.label_sent_at,
        new.label_idempotency_key,new.label_fingerprint)
        is distinct from
        (old.recipient_name,old.line1,old.line2,old.city,old.region,old.postal_code,
        old.country,old.phone,old.seller_instructions,old.carrier_name,old.service_level,
        old.tracking_number,old.tracking_url,old.buyer_note,old.instructions_provided_at,
        old.shipped_at,old.seller_instruction_idempotency_key,
        old.seller_instruction_fingerprint,old.buyer_shipping_idempotency_key,
        old.buyer_shipping_fingerprint,old.destination_fingerprint,
        old.shipped_destination_fingerprint,old.return_label_asset_id,old.label_sent_at,
        old.label_idempotency_key,old.label_fingerprint)
      or new.received_at is null or new.received_by<>new.seller_id
      or new.seller_receipt_idempotency_key is null
      or new.seller_receipt_fingerprint is null then
      raise exception using errcode='42501',message='marketplace_return_receipt_transition_denied';
    end if;
    return new;
  end if;
...
alter table public.marketplace_return_refunds
  drop constraint marketplace_return_refunds_mode_check;
alter table public.marketplace_return_refunds
  add constraint marketplace_return_refunds_mode_check
  check(resolution_mode in('keep_item','returned_item'));
...
alter table public.marketplace_order_events
  add constraint marketplace_order_events_type_check check(event_type in(
    ...
    'return_instructions_provided','return_label_sent','return_shipped','return_received'
  ));
alter table public.marketplace_order_events
...
select case when rs.id is null then null else jsonb_build_object(
  ...
  'seller_receipt_note',rs.seller_receipt_note
)end
from (select 1) seed
...
declare
  ...
  v_now timestamptz:=clock_timestamp();
begin
  if p_return_id is null or p_seller_id is null or p_idempotency_key is null
    or p_resolution_mode is null or p_resolution_mode not in('keep_item','returned_item')
    or(v_note is not null and(
      char_length(v_note)>case when p_resolution_mode='returned_item' then 500 else 1000 end
      or v_note~*'<[[:space:]]*/?[[:alpha:]][^>]*>'
    ))then
    raise exception using errcode='22023',message='marketplace_return_decision_invalid_input';
  end if;
  v_fingerprint:=encode(extensions.digest(convert_to(jsonb_build_object(
    'return_id',p_return_id,'resolution_mode',p_resolution_mode,'seller_note',v_note
  )::text,'UTF8'),'sha256'),'hex');

  perform pg_advisory_xact_lock(hashtextextended(
    case when p_resolution_mode='keep_item'
      then'marketplace-return-keep-item-refund:'
      else'marketplace-return-received-refund:'end||p_return_id::text,0));
  ...
  if not exists(select 1 from public.marketplace_sellers se
      where se.user_id=p_seller_id and se.status='approved')
    or not exists(select 1 from public.marketplace_stores st
  ...
  if found then
    if prior.return_request_id<>rr.id or prior.request_fingerprint<>v_fingerprint
      or prior.resolution_mode<>p_resolution_mode then
  ...
  if rr.status<>'approved' or o.id is null or o.status<>'delivered'
    ...
    or h.currency<>'BDAG'
    or(h.return_request_id,h.settlement_id,h.payment_id,h.allocation_id,h.checkout_id,
      h.order_id,h.buyer_id,h.seller_id,h.store_id,h.gross_amount)
      is distinct from(rr.id,s.id,p.id,a.id,o.checkout_id,o.id,o.buyer_id,o.seller_id,
      o.store_id,s.gross_amount)then
    raise exception using errcode='55000',message='marketplace_return_refund_not_eligible';
  end if;
  if exists(select 1 from public.marketplace_return_refunds rf where rf.return_request_id=rr.id)
    or exists(select 1 from public.marketplace_settlement_reversals rv
  ...
  if p_resolution_mode='keep_item' then
  ...
  if not exists(select 1 from public.ledger_accounts la
    where la.id=v_buyer_account and la.owner_id=o.buyer_id
      and la.account_type='user' and la.currency='BDAG' and not la.frozen)then
    raise exception using errcode='23514',message='marketplace_return_refund_account_invalid';
  end if;
  ...
  insert into public.financial_transactions(
    ...
    p_idempotency_key::text,p_seller_id
  );
  insert into public.marketplace_return_refunds(
    ...
    h.gross_amount,'BDAG',p_resolution_mode,v_tx_id,p_idempotency_key,v_fingerprint,v_now,v_now
  );
  perform public.ledger_debit(v_tx_id,v_return_escrow,h.gross_amount,
    case when p_resolution_mode='returned_item'
      then'Marketplace returned-item refund'else'Marketplace keep-item return refund'end,
    jsonb_build_object('return_request_id',rr.id,'refund_hold_id',h.id,
      'return_refund_id',v_refund_id,'resolution_mode',p_resolution_mode));
  ...
  if p_resolution_mode='returned_item' then
    perform set_config('app.marketplace_return_receipt','on',true);
    update public.marketplace_return_shipments set
      status='received',received_at=v_now,received_by=p_seller_id,
      seller_receipt_note=v_note,seller_receipt_idempotency_key=p_idempotency_key,
      seller_receipt_fingerprint=v_fingerprint,updated_at=v_now
    where id=rs.id and status='shipped' and seller_id=p_seller_id;
    get diagnostics v_rows=row_count;
    perform set_config('app.marketplace_return_receipt','off',true);
    if v_rows<>1 then
      raise exception using errcode='23514',message='marketplace_return_refund_state_transition_failed';
    end if;
  end if;
  ...
  if v_rows<>1 then raise exception using errcode='23514',message='marketplace_return_refund_state_transition_failed';end if;
  perform set_config('app.marketplace_return_refund','off',true);
  ...
  if p_resolution_mode='returned_item' then
    insert into public.marketplace_order_events(
      ...
      o.id,o.checkout_id,o.buyer_id,o.seller_id,o.store_id,'return_received',
      o.status,o.status,p_seller_id,'seller','marketplace_return_received',p_idempotency_key,
      jsonb_build_object(
        'return_request_id',rr.id,'return_shipment_id',rs.id,
        'return_refund_id',v_refund_id,'refund_hold_id',h.id,
        'buyer_refund_transaction_id',v_tx_id,'gross_refund_amount',h.gross_amount,
        'resolution_mode','returned_item','request_fingerprint',v_fingerprint,
        'money_moved',true
      ),v_now
    );
  end if;
  insert into public.marketplace_order_events(
    ...
    o.status,'refunded',p_seller_id,'seller',
    case when p_resolution_mode='returned_item'
      then'marketplace_return_received_refund'
      else'marketplace_return_refund_without_shipment'end,
    p_idempotency_key,jsonb_build_object(
      'return_request_id',rr.id,'return_shipment_id',case when rs.id is null then null else rs.id end,
      'return_refund_id',v_refund_id,'refund_hold_id',h.id,
      'buyer_refund_transaction_id',v_tx_id,'gross_refund_amount',h.gross_amount,
      'resolution_mode',p_resolution_mode,'request_fingerprint',v_fingerprint,
      'money_moved',true
    ),v_now
  );
...
create or replace function public.refund_marketplace_return_without_shipment(
...
begin
  ...
  if found then
    if prior.return_request_id<>rr.id or prior.request_fingerprint<>v_fingerprint
      or prior.resolution_mode<>'keep_item' then
      raise exception using errcode='23505',message='marketplace_return_refund_idempotency_conflict';
    end if;
    return public.marketplace_return_refund_hold_receipt(rr.id,false);
  end if;
  ...
  if rr.status not in('requested','approved')
    or exists(select 1 from public.marketplace_return_shipments rs
      where rs.return_request_id=rr.id)then
    raise exception using errcode='55000',message='marketplace_return_refund_not_eligible';
  end if;
  if not exists(select 1 from public.marketplace_sellers se
      where se.user_id=v_actor and se.status='approved')
  ...
  return public.marketplace_complete_return_refund_core(
    rr.id,v_actor,'keep_item',v_note,p_idempotency_key
  );
end;
$$;

create function public.confirm_marketplace_return_received(
  p_return_id uuid,p_seller_note text,p_idempotency_key uuid
)returns jsonb language plpgsql security definer set search_path=pg_catalog,public as $$
declare
  v_actor uuid:=auth.uid();
...
begin
  ...
  if p_return_id is null or p_idempotency_key is null
    or(v_note is not null and(char_length(v_note)>500
      or v_note~*'<[[:space:]]*/?[[:alpha:]][^>]*>'))then
    raise exception using errcode='22023',message='marketplace_return_receipt_invalid_input';
  end if;
  select * into rr from public.marketplace_return_requests where id=p_return_id;
  if not found then
    raise exception using errcode='P0002',message='marketplace_return_not_found';
  end if;
  if rr.seller_id<>v_actor then
  ...
  return public.marketplace_complete_return_refund_core(
    rr.id,v_actor,'returned_item',v_note,p_idempotency_key
  );
end;
$$;
...
begin
  ...
  if auth.uid() not in(o.buyer_id,o.seller_id)then
    raise exception using errcode='42501',message='marketplace_order_not_owned';
  end if;
  return jsonb_build_object(
    'shipping_amount',o.shipping_amount,
    'shipping',(select jsonb_build_object('estimated_delivery_at',sh.estimated_delivery_at)
      from public.marketplace_order_shipments sh where sh.order_id=o.id),
    'shipping_snapshot',(select jsonb_build_object(
      'processing_days_min',min(s.processing_days_min),'processing_days_max',max(s.processing_days_max),
      'transit_days_min',min(s.transit_days_min),'transit_days_max',max(s.transit_days_max),
      'return_policy_summary',max(s.return_policy_summary))
      from public.marketplace_order_shipping_snapshots s where s.order_id=o.id),
    'dispute',(select jsonb_build_object(
      'id',d.id,'status',d.status,'reason_code',d.reason_code,'buyer_note',d.buyer_note,
      'created_at',d.created_at,'outcome',x.outcome,'decided_at',x.decided_at,
      ...
      'seller_response',case when auth.uid()=o.seller_id then(
        select jsonb_build_object('id',r.id,'note',r.note,'created_at',r.created_at,
      ...
      left join public.marketplace_dispute_decisions x on x.dispute_id=d.id
      where d.order_id=o.id order by d.created_at desc limit 1),
    'return_eligible',auth.uid()=o.buyer_id and o.status='delivered'
      and(select count(*)from public.marketplace_order_settlements se where se.order_id=o.id)=1
      and exists(select 1 from public.marketplace_payments p
    ...
    'return_request',(select jsonb_build_object(
      ...
      'created_at',rr.created_at,'decided_at',rr.decided_at,
      'refund_hold',(select jsonb_build_object('status',h.status,'gross_amount',h.gross_amount,
        'held_at',h.held_at)from public.marketplace_return_refund_holds h
        where h.return_request_id=rr.id),
      'refund',(select jsonb_build_object('mode',rf.resolution_mode,'gross_amount',rf.gross_amount,
        'refunded_at',rf.refunded_at)from public.marketplace_return_refunds rf
...
begin
  ...
  return coalesce((select jsonb_agg(x.row order by x.created_at desc,x.id desc)from(
    select o.created_at,o.id,jsonb_build_object(
      'id',o.id,'order_number',o.order_number,'checkout_id',o.checkout_id,
      'checkout_reference',c.reference,'status',o.status,'store_id',o.store_id,
      'store_name',st.name,'total',o.total,'currency',o.currency,'created_at',o.created_at,
      'confirmed_at',o.confirmed_at,'processing_at',o.processing_at,'shipped_at',o.shipped_at,
      'delivered_at',o.delivered_at,
      'first_item_title',(select i.product_title from public.marketplace_order_items i
        where i.order_id=o.id order by i.created_at limit 1),
      'first_item_image',(select i.image_url from public.marketplace_order_items i
        where i.order_id=o.id order by i.created_at limit 1),
      ...
      where rr.order_id=o.id and rr.buyer_id=auth.uid() and rr.status in('approved','refunded'))
    )row from public.marketplace_orders o
...
begin
  if auth.uid() is null then raise exception using errcode='42501',message='marketplace_auth_required';end if;
  if not exists(select 1 from public.marketplace_sellers where user_id=auth.uid() and status='approved')then
    raise exception using errcode='42501',message='marketplace_seller_not_approved';end if;
  ...
  return coalesce((select jsonb_agg(x.row order by x.created_at desc,x.id desc)from(
    select o.created_at,o.id,jsonb_build_object(
      ...
      'store_name',st.name,'total',o.total,'currency',o.currency,'created_at',o.created_at,
      'confirmed_at',o.confirmed_at,'processing_at',o.processing_at,'shipped_at',o.shipped_at,
      'delivered_at',o.delivered_at,'recipient_name',sa.recipient_name,'city',sa.city,
      'region',sa.region,'country',sa.country,
      'distinct_lines',(select count(*)from public.marketplace_order_items i where i.order_id=o.id),
      'total_quantity',(select sum(i.quantity)from public.marketplace_order_items i where i.order_id=o.id),
      'gross_amount',a.gross_amount,'platform_fee_amount',a.platform_fee_amount,
      'seller_net_amount',a.seller_net_amount,'allocation_status',a.status,
      'released_at',a.released_at,'carrier_name',sh.carrier_name,'tracking_number',sh.tracking_number,
      'active_dispute',(select jsonb_build_object(
        'id',d.id,'status',d.status,'reason_code',d.reason_code,'created_at',d.created_at,
        'seller_response_submitted',exists(select 1 from public.marketplace_dispute_seller_responses sr
          where sr.dispute_id=d.id))from public.marketplace_order_disputes d
...
begin
  if v_actor is null then raise exception using errcode='42501',message='marketplace_auth_required';end if;
  if not exists(select 1 from public.marketplace_sellers where user_id=v_actor and status='approved')then
    raise exception using errcode='42501',message='marketplace_seller_not_approved';end if;
  select id into v_store from public.marketplace_stores where seller_id=v_actor and status='active';
  if v_store is null then raise exception using errcode='42501',message='marketplace_store_inactive';end if;
  if p_limit is null or p_limit<1 or p_limit>50 then
    raise exception using errcode='22023',message='marketplace_invalid_limit';end if;
  if(p_before_created_at is null)<>(p_before_id is null)then
    raise exception using errcode='22023',message='marketplace_invalid_cursor';end if;
  v_limit:=p_limit;
  ...
  select jsonb_build_object(
    ...
    'returns',coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'return_shipping_status',shipping_status,'order_id',order_id,'order_number',order_number,
      'order_status',order_status,'store_id',store_id,'store_name',store_name
    ...
    'next_cursor',case when(select count(*)from paged)>v_limit then(
      ...
      from selected order by created_at asc,id asc limit 1)else null end
  ));
end;
$$;
...
returns jsonb language sql stable security definer set search_path=pg_catalog,public as $$
with escrow as(
  select(coalesce((select sum(h.gross_amount)from public.marketplace_return_refund_holds h
...
select jsonb_build_object(
  ...
  'duplicate_original_leg',(select count(*)from(select original_settlement_leg_id
    from public.marketplace_return_refund_hold_legs group by 1 having count(*)>1)x),
  ...
  'transaction_reference_mismatch',(select count(*)from public.marketplace_return_refund_hold_legs l
    ...
    where f.reference_type<>'marketplace_return_refund_hold' or f.reference_id<>h.id::text
      ...
      or f.operation_type<>case l.leg_type when'seller_net'then'marketplace_return_seller_hold'
  ...
  'refund_identity_mismatch',(select count(*)from public.marketplace_return_refunds r
    ...
    where(r.return_request_id,r.settlement_id,r.payment_id,r.allocation_id,r.order_id,
      ...
      h.buyer_id,h.seller_id,h.store_id,h.gross_amount,h.currency)or rr.status<>'refunded'),
  ...
  'refund_resolution_mismatch',(select count(*)from public.marketplace_return_refunds r
    left join public.marketplace_return_shipments rs on rs.return_request_id=r.return_request_id
    where(r.resolution_mode='keep_item'and rs.id is not null)
      or(r.resolution_mode='returned_item'and(rs.id is null or rs.status<>'received'))),
  'returned_item_refund_without_received_shipment',(select count(*)
    from public.marketplace_return_refunds r
    left join public.marketplace_return_shipments rs on rs.return_request_id=r.return_request_id
    where r.resolution_mode='returned_item'and(rs.id is null or rs.status<>'received')),
  'receipt_transaction_mismatch',(select count(*)from public.marketplace_return_refunds r
    join public.marketplace_return_shipments rs on rs.return_request_id=r.return_request_id
    left join public.financial_transactions f on f.id=r.financial_transaction_id
    where r.resolution_mode='returned_item'and(f.id is null or f.initiated_by is distinct from rs.received_by
      or f.amount<>r.gross_amount or f.reference_id<>r.id::text)),
...
select jsonb_build_object(
  ...
  'return_identity_mismatch',(select count(*)from public.marketplace_return_shipments rs
    ...
    where(rr.order_id,rr.buyer_id,rr.seller_id,rr.store_id)
      is distinct from(rs.order_id,rs.buyer_id,rs.seller_id,rs.store_id)
      or(o.buyer_id,o.seller_id,o.store_id)is distinct from(rs.buyer_id,rs.seller_id,rs.store_id)),
  'unfunded_return_shipment',(select count(*)from public.marketplace_return_shipments rs
    ...
    where rr.status not in('approved','refunded')or h.id is null),
  'missing_return_label_link',(select count(*)from public.marketplace_return_shipments rs
    where rs.return_label_asset_id is not null and not exists(select 1
      from public.media_asset_links l where l.asset_id=rs.return_label_asset_id
  ...
  'received_without_refund',(select count(*)from public.marketplace_return_shipments rs
    left join public.marketplace_return_refunds r on r.return_request_id=rs.return_request_id
    where rs.status='received'and(r.id is null or r.resolution_mode<>'returned_item')),
  'returned_item_refund_without_received_shipment',(select count(*)
    from public.marketplace_return_refunds r
    left join public.marketplace_return_shipments rs on rs.return_request_id=r.return_request_id
    where r.resolution_mode='returned_item'and(rs.id is null or rs.status<>'received')),
  'receipt_identity_mismatch',(select count(*)from public.marketplace_return_shipments rs
    join public.marketplace_return_refunds r on r.return_request_id=rs.return_request_id
    where rs.status='received'and(r.resolution_mode<>'returned_item'
      or(rs.return_request_id,rs.order_id,rs.buyer_id,rs.seller_id,rs.store_id,rs.received_by)
        is distinct from(r.return_request_id,r.order_id,r.buyer_id,r.seller_id,r.store_id,r.seller_id))),
  'receipt_transaction_mismatch',(select count(*)from public.marketplace_return_shipments rs
    join public.marketplace_return_refunds r on r.return_request_id=rs.return_request_id
    left join public.financial_transactions f on f.id=r.financial_transaction_id
    where rs.status='received'and(f.id is null or f.operation_type<>'marketplace_return_refund'
      or f.amount<>r.gross_amount or f.currency<>'BDAG'or f.status<>'completed'
      or f.reference_type<>'marketplace_return_refund'or f.reference_id<>r.id::text
      or f.initiated_by is distinct from rs.received_by)),
  'receipt_event_mismatch',(select count(*)from public.marketplace_return_shipments rs
    join public.marketplace_return_refunds r on r.return_request_id=rs.return_request_id
    where rs.status='received'and(
      not exists(select 1 from public.marketplace_order_events e
        where e.order_id=rs.order_id and e.event_type='return_received'
          and e.idempotency_key=rs.seller_receipt_idempotency_key
          and e.metadata->>'return_shipment_id'=rs.id::text
          and e.metadata->>'return_refund_id'=r.id::text
          and e.metadata->>'resolution_mode'='returned_item')
      or not exists(select 1 from public.marketplace_order_events e
        where e.order_id=rs.order_id and e.event_type='refund_created'
          and e.idempotency_key=rs.seller_receipt_idempotency_key
          and e.metadata->>'return_refund_id'=r.id::text
          and e.metadata->>'resolution_mode'='returned_item'))),
  'invalid_received_state',(select count(*)from public.marketplace_return_shipments rs
    where(rs.status='received'and(rs.received_at is null or rs.received_by<>rs.seller_id
      or rs.seller_receipt_idempotency_key is null or rs.seller_receipt_fingerprint is null
      or rs.shipped_at is null or rs.buyer_shipping_idempotency_key is null
      or rs.buyer_shipping_fingerprint is null))
      or(rs.status<>'received'and(rs.received_at is not null or rs.received_by is not null
        or rs.seller_receipt_note is not null or rs.seller_receipt_idempotency_key is not null
        or rs.seller_receipt_fingerprint is not null))),
  'return_shipment_count',(select count(*)from public.marketplace_return_shipments)
);
$$;

comment on function public.marketplace_complete_return_refund_core(uuid,uuid,text,text,uuid) is
...
revoke all on function public.marketplace_complete_return_refund_core(uuid,uuid,text,text,uuid)
...
revoke all on function public.reconcile_marketplace_return_refund_holds()
  from public,anon,authenticated;
revoke all on function public.reconcile_marketplace_return_shipments()
  from public,anon,authenticated;
grant execute on function public.reconcile_marketplace_return_refund_holds() to service_role;
grant execute on function public.reconcile_marketplace_return_shipments() to service_role;
