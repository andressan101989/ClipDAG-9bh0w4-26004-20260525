...
-- Preserve the complete C3-C1-C1 lock closure and global order.  Every
-- explicit table lock is at least as strong as the automatic mode required
-- by the canonical path.  In particular, FOR KEY SHARE on the rule set
-- requires ROW SHARE, which must be acquired with NOWAIT inside the budget.
...
begin
  ...
  -- Authorization follows the NOWAIT table prelocks but precedes every host,
  -- session, Battle, score, request, projection, rule-set and power row lock.
...
-- Reinstall the public RPC unchanged so its SECURITY DEFINER identity, narrow
-- lock_not_available retry envelope and canonical transition path remain the
-- sole authority after the helper replacement.
