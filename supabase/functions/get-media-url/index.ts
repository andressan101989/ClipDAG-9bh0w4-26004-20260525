import { authenticatedClient,authenticatedUser,admin,json } from '../_shared/mediaAuth.ts';
...
Deno.serve(async(req)=>{
  ...
  if(a.visibility==='private'&&a.owner_id!==user.id
    ...
    &&!(await adminMayReadDisputeEvidence(req,a.id))) return json({error:'forbidden'},403);
