...
const groups = [ ['c1', /^liveBattlesLb4F9AC1/], ['f9-a', /^liveBattlesLb4F9AScoring/], ['f8', /Lb4F8/],
...
const baseline = normalize(readFileSync('docs/validation/lb4-f9-a/typescript.txt','utf8'));
...
const base = 'adb741677a8d80f9bb9d54ba3a0572c22cd4964e';
...
const protectedFiles = ['package.json', 'package-lock.json', ...readdirSync('supabase/migrations').filter(f => f.endsWith('.sql') && !migration.endsWith(f)).map(f => 'supabase/migrations/' + f)];
assert.equal(protectedFiles.length, 212);
...
console.log('212 protected LF hashes unchanged; F9-A:', finalHash);
