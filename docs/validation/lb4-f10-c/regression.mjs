...
const outputDir = mkdtempSync(join(tmpdir(), 'clipdag-lb4-f10-c-'));

try {
  const files = readdirSync('tests').filter(f => f.endsWith('.test.mjs')).sort();
  const groups = [ ['f10-c', /^liveBattlesLb4F10C/], ['f10-b', /^liveBattlesLb4F10B/], ['f10-a', /^liveBattlesLb4F10A/], ['media-relay', /Relay|PostRound/i], ['host-ui', /F2Invitation|F8BPhysical|F5BClient|Ui1Figma/i], ['realtime', /Realtime|Subscription|Snapshot|Projection/i], ['agora-auth', /Agora|agoraToken/i], ['f9-b', /^liveBattlesLb4F9BSpectatorControls/], ['viewer-ui', /Lb4F7BViewerUi|Lb4Ui1FigmaStage/], ['gifts', /gift/i], ['c1', /^liveBattlesLb4F9AC1/], ['f9-a', /^liveBattlesLb4F9AScoring/], ['f8', /Lb4F8/],
  ...
  for (const [name, pattern] of (process.argv[2] === 'verify' ? [] : groups)) {
    ...
    writeFileSync(join(outputDir, name + '.tap'), output);
    ...
    writeFileSync(join(outputDir, 'suites.json'), JSON.stringify(results, null, 2));
  ...
  const typescriptOutput = tsc.stdout + tsc.stderr;
  writeFileSync(join(outputDir, 'typescript.txt'), typescriptOutput);
  ...
  const baseline = normalize(readFileSync('docs/validation/lb4-f10-b/typescript.txt','utf8'));
  const current = normalize(typescriptOutput);
  ...
  writeFileSync(join(outputDir, 'typescript-comparison.json'), JSON.stringify({ exitCode: tsc.status, historical: old.length, current: now.length, added, removed, exactNormalizedOutputMatch: baseline === current }, null, 2));
  ...
  const base = 'fdee4a8dd7eb34af8632c3e8885f1d9a1d431917';
  ...
  writeFileSync(join(outputDir, 'protected-lf-hashes.json'), JSON.stringify(hashes, null, 2));
  const finalHash = hash(readFileSync(migration,'utf8'));
  writeFileSync(join(outputDir, 'migration-lf-hash.json'), JSON.stringify({ file: migration, sha256LF: finalHash }, null, 2));
