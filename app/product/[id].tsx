...
import {
  ...
  type MarketplaceProductOption,
  type MarketplaceProductGalleryItem,
...
export default function ProductScreen() {
  ...
  const [gallery, setGallery] = useState<MarketplaceProductGalleryItem[]>([]);
  const [selectedValues, setSelectedValues] = useState<Record<string, string>>(
    {},
  );
  const [isLoading, setIsLoading] = useState(true);
  const [notFound, setNotFound] = useState(false);
  const [readError, setReadError] = useState<"permission" | "transport" | null>(
    null,
  );
      ...
      .catch((error) => {
        if (active) {
          if (
            ...
            setReadError("permission");
          else if (
            error instanceof MarketplaceReadError &&
            error.code === "marketplace_read_transport"
          )
            setReadError("transport");
  ...
  const selectedMedia = gallery[selectedImageIndex] ?? null;
  const videoPlayer = useVideoPlayer(
    selectedMedia?.kind === "video" ? selectedMedia.url : null,
    (player) => {
      player.muted = true;
    },
  );
    ...
    return (
      ...
      >
        ...
        <Text style={[styles.description, { textAlign: "center" }]}>
          {readError === "permission"
            ? "La tienda necesita una actualización de configuración. Inténtalo nuevamente."
            : readError === "transport"
  ...
  const imageGallery = gallery.filter((item) => item.kind === "image");
  const displayImage =
    selectedVariant?.image_url ??
    (selectedMedia?.kind === "image"
      ? selectedMedia.url
      : (imageGallery[0]?.url ?? product.images[selectedImageIndex]));
  ...
  const handleAddToCart = () => {
    ...
    try {
      ...
      const optionText = selectedOptions
        ...
        .join(" · ");
      Alert.alert(
        adjusted ? "Cantidad ajustada" : "Agregado al carrito",
        adjusted
          ? `Solo hay ${result.applied} unidades disponibles para esta variante.`
          : `${product.title}${optionText ? ` · ${optionText}` : ""} · Cantidad ${result.item.quantity}`,
        [
          { text: "Seguir comprando", style: "cancel" },
          { text: "Ver carrito", onPress: () => router.push("/cart" as never) },
        ],
      );
    } finally {
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
                  ...
                  >
                    {item.kind === "image" ? (
                      <Image
                        ...
                        style={[
                          styles.thumbImg,
                          i === selectedImageIndex && styles.thumbImgActive,
                        ]}
                        contentFit="cover"
                      />
                    ) : (
        ...
        <View style={styles.infoSection}>
          ...
          <View style={styles.priceRow}>
            <Text style={styles.price}>
              {options.length > 0 && !selectedVariant && hasDifferentPrices
                ? "Desde "
                : ""}
              {effectivePrice.toFixed(2)} BDAG
            </Text>
            {selectedVariant?.compare_at_price != null &&
            selectedVariant.compare_at_price > effectivePrice ? (
              <Text style={styles.comparePrice}>
                {selectedVariant.compare_at_price.toFixed(2)} BDAG
              </Text>
            ) : null}
              ...
              {option.values.length > 6 ? (
                <SearchableSelectField
                  ...
                  options={option.values
                    .filter((value) =>
                      isOptionValueSelectable(variants, value.id),
                    )
                    ...
                    return (
                      <Pressable
                        ...
                        style={[
                          ...
                          !enabled && styles.optionChipDisabled,
                        ]}
          ...
          <View style={styles.stockRow}>
            <View
              style={[
                styles.stockDot,
                available > 0 ? styles.stockDotAvail : styles.stockDotOut,
              ]}
            />
            <Text
              style={[
                styles.stockText,
                available === 0 && { color: Colors.secondary },
              ]}
            >
              {selectionIncomplete
      ...
      {!isOwner && selectedVariant && available > 0 ? (
        <View
          style={[styles.buyBar, { paddingBottom: insets.bottom + Spacing.sm }]}
        >
          {/* Quantity */}
...
const styles = StyleSheet.create({
  ...
  videoThumbText: { color: "#fff", fontSize: 9, fontWeight: FontWeight.bold },
