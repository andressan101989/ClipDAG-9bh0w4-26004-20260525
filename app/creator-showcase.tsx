...
export default function CreatorShowcaseScreen() {
  ...
  const add = useCallback(async (product: MarketplaceCreatorShowcaseProduct) => {
    ...
    try {
      ...
      if (error instanceof MarketplaceCreatorShowcaseError && error.code === "marketplace_creator_showcase_limit_reached") {
        ...
        Alert.alert("Producto no disponible", "La oferta del vendedor ya no es elegible para tu escaparate.");
      }
    } finally {
      setBusyId(null);
    }
  ...
  const remove = useCallback((item: MarketplaceCreatorShowcaseManagementItem) => {
    if (busyId) return;
    Alert.alert(
      "Quitar del escaparate",
      `${item.title} dejará de aparecer en tu escaparate público. La atribución histórica no cambiará.`,
      [
          ...
          onPress: () => {
            ...
            void (async () => {
              try {
                ...
                Alert.alert("No pudimos quitar el producto", "Inténtalo de nuevo.");
              } finally {
                setBusyId(null);
              }
            })();
  ...
  const move = useCallback(async (item: MarketplaceCreatorShowcaseManagementItem, delta: -1 | 1) => {
    ...
    try {
      ...
      Alert.alert("No pudimos reordenar el escaparate", "Actualiza e inténtalo de nuevo.");
    } finally {
      setBusyId(null);
    }
  }, [active, busyId, load]);
    ...
    <KeyboardAvoidingView style={styles.root} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      ...
      <View style={[styles.header, { paddingTop: insets.top }]}> 
        ...
        <View style={styles.headerCopy}>
          <Text style={styles.title}>Escaparate de Creator</Text>
          <Text style={styles.subtitle}>Productos con ofertas aprobadas por vendedores · {active.length} / 100</Text>
        </View>
      ...
      <View style={styles.tabs}>
        <TabButton active={tab === "showcase"} label="Mi escaparate" icon="storefront-outline" onPress={() => setTab("showcase")} />
        <TabButton active={tab === "eligible"} label="Disponibles" icon="shopping-search" onPress={() => setTab("eligible")} />
      </View>
      {tab === "eligible" ? (
        <View style={styles.searchWrap}>
          <MaterialIcons name="search" size={20} color={Colors.textSubtle} />
          <TextInput value={search} onChangeText={setSearch} placeholder="Buscar productos o tiendas" placeholderTextColor={Colors.textSubtle} style={styles.search} returnKeyType="search" accessibilityLabel="Buscar productos elegibles" />
        </View>
      ) : null}
      {loading && !data.length ? <View style={styles.center}><ActivityIndicator color={Colors.primary} /><Text style={styles.muted}>Cargando productos…</Text></View>
        : error ? <View style={styles.center}><MaterialIcons name="cloud-off" size={42} color={Colors.textSubtle} /><Text style={styles.emptyTitle}>No pudimos cargar tu escaparate</Text><Pressable style={styles.retry} onPress={() => void load()} accessibilityRole="button"><Text style={styles.retryText}>Reintentar</Text></Pressable></View>
          : <FlatList
              data={data}
              ...
              ListEmptyComponent={<View style={styles.center}><MaterialCommunityIcons name={tab === "showcase" ? "storefront-outline" : "shopping-search"} size={48} color={Colors.borderHighlight} /><Text style={styles.emptyTitle}>{tab === "showcase" ? "Tu escaparate está vacío" : "No hay productos elegibles"}</Text><Text style={styles.muted}>{tab === "showcase" ? "Elige un producto con oferta aprobada por el vendedor para comenzar." : "Prueba otra búsqueda o vuelve cuando los vendedores publiquen nuevas ofertas."}</Text>{tab === "showcase" ? <Pressable style={styles.retry} onPress={() => setTab("eligible")} accessibilityRole="button"><Text style={styles.retryText}>Explorar productos disponibles</Text></Pressable> : null}</View>}
              renderItem={({ item, index }) => {
                ...
                return <View style={styles.card}>
                  <Pressable style={styles.productTap} onPress={() => router.push({ pathname: "/product/[id]", params: { id: item.productId } } as never)} accessibilityRole="button" accessibilityLabel={`Abrir ${item.title}`}>
                    ...
                    <View style={styles.copy}>
                      ...
                      {tab === "eligible" && item.commissionBps ? <View style={styles.commission}><MaterialCommunityIcons name="hand-coin-outline" size={14} color={Colors.accent} /><Text style={styles.commissionText}>{(item.commissionBps / 100).toFixed(2)}% de comisión</Text></View> : null}
                      {management && !management.currentEligible ? <Text style={styles.unavailable}>Oferta finalizada · no disponible para compradores</Text> : null}
                    </View>
                  </Pressable>
                  {management ? <View style={styles.actions}>
                    <Pressable style={[styles.action, (index === 0 || busy) && styles.disabled]} disabled={index === 0 || busy} onPress={() => void move(management, -1)} accessibilityRole="button" accessibilityLabel={`Mover ${item.title} hacia arriba`} accessibilityState={{ disabled: index === 0 || busy }}><MaterialIcons name="keyboard-arrow-up" size={24} color={index === 0 ? Colors.textSubtle : Colors.textPrimary} /></Pressable>
                    <Pressable style={[styles.action, (index === active.length - 1 || busy) && styles.disabled]} disabled={index === active.length - 1 || busy} onPress={() => void move(management, 1)} accessibilityRole="button" accessibilityLabel={`Mover ${item.title} hacia abajo`} accessibilityState={{ disabled: index === active.length - 1 || busy }}><MaterialIcons name="keyboard-arrow-down" size={24} color={index === active.length - 1 ? Colors.textSubtle : Colors.textPrimary} /></Pressable>
                    <Pressable style={[styles.action, busy && styles.disabled]} disabled={busy} onPress={() => void remove(management)} accessibilityRole="button" accessibilityLabel={`Quitar ${item.title} del escaparate`} accessibilityState={{ disabled: busy }}><MaterialIcons name="delete-outline" size={22} color={Colors.secondary} /></Pressable>
                  </View> : <Pressable style={[styles.add, (item.selected || busy) && styles.disabled]} disabled={item.selected || busy} onPress={() => void add(item)} accessibilityRole="button" accessibilityLabel={item.selected ? `${item.title} ya está seleccionado` : `Agregar ${item.title} al escaparate`} accessibilityState={{ disabled: item.selected || busy }}>
...
const styles = StyleSheet.create({
  root:{flex:1,backgroundColor:Colors.bg},header:{minHeight:76,flexDirection:"row",alignItems:"flex-end",paddingHorizontal:Spacing.sm,paddingBottom:Spacing.sm,borderBottomWidth:1,borderBottomColor:Colors.border},iconButton:{width:44,height:44,alignItems:"center",justifyContent:"center"},headerCopy:{flex:1,minWidth:0,alignItems:"center"},title:{color:Colors.textPrimary,fontSize:FontSize.xl,fontWeight:FontWeight.extrabold,textAlign:"center"},subtitle:{color:Colors.textSubtle,fontSize:FontSize.xs,textAlign:"center"},tabs:{flexDirection:"row",padding:Spacing.sm,gap:Spacing.sm},tab:{flex:1,minHeight:44,borderRadius:Radius.full,flexDirection:"row",alignItems:"center",justifyContent:"center",gap:Spacing.xs,backgroundColor:Colors.surface,borderWidth:1,borderColor:Colors.border},tabActive:{backgroundColor:Colors.primaryDim,borderColor:Colors.primary},tabText:{color:Colors.textSubtle,fontWeight:FontWeight.semibold},tabTextActive:{color:Colors.textPrimary},searchWrap:{marginHorizontal:Spacing.md,marginBottom:Spacing.sm,minHeight:46,borderRadius:Radius.md,backgroundColor:Colors.surfaceElevated,borderWidth:1,borderColor:Colors.border,flexDirection:"row",alignItems:"center",paddingHorizontal:Spacing.md,gap:Spacing.sm},search:{flex:1,minWidth:0,color:Colors.textPrimary,fontSize:FontSize.md},list:{padding:Spacing.md,gap:Spacing.md},emptyList:{flexGrow:1},center:{flex:1,minHeight:260,alignItems:"center",justifyContent:"center",gap:Spacing.md,padding:Spacing.xl},muted:{color:Colors.textSecondary,textAlign:"center",fontSize:FontSize.sm,lineHeight:20},emptyTitle:{color:Colors.textPrimary,fontSize:FontSize.lg,fontWeight:FontWeight.bold,textAlign:"center"},retry:{minHeight:44,paddingHorizontal:Spacing.lg,borderRadius:Radius.full,backgroundColor:Colors.primary,alignItems:"center",justifyContent:"center"},retryText:{color:"#fff",fontWeight:FontWeight.bold},card:{backgroundColor:Colors.surfaceElevated,borderRadius:Radius.lg,borderWidth:1,borderColor:Colors.border,padding:Spacing.sm,flexDirection:"row",alignItems:"center",gap:Spacing.sm},productTap:{flex:1,minWidth:0,flexDirection:"row",alignItems:"center",gap:Spacing.md},image:{width:82,height:82,borderRadius:Radius.md},imageFallback:{backgroundColor:Colors.surface,alignItems:"center",justifyContent:"center"},copy:{flex:1,minWidth:0,gap:3},productTitle:{color:Colors.textPrimary,fontSize:FontSize.md,fontWeight:FontWeight.bold},store:{color:Colors.textSubtle,fontSize:FontSize.xs},price:{color:Colors.primaryLight,fontWeight:FontWeight.extrabold},commission:{flexDirection:"row",alignItems:"center",gap:4},commissionText:{color:Colors.accent,fontSize:FontSize.xs,fontWeight:FontWeight.semibold},unavailable:{color:Colors.warning,fontSize:FontSize.xs},add:{width:44,height:44,borderRadius:22,backgroundColor:Colors.primary,alignItems:"center",justifyContent:"center"},disabled:{opacity:.45},actions:{gap:2,flexShrink:0},action:{width:44,height:44,alignItems:"center",justifyContent:"center"},
});
