...
export type MediaClientStage =
  | 'MEDIA_INPUT' | 'MEDIA_NORMALIZE_IMAGE' | 'MEDIA_CREATE_UPLOAD_RECORD'
  | 'MEDIA_R2_PUT' | 'MEDIA_FINALIZE' | 'MEDIA_DELETE' | 'MEDIA_UNKNOWN';
type ErrorLike={name?:unknown;message?:unknown;code?:unknown;details?:unknown;hint?:unknown;status?:unknown;httpStatus?:unknown};
...
export function getSafeMediaError(
  ...
  const message=safeText(existing?.message??source.message)??'media_operation_failed';
  const abort=source.name==='AbortError'||message.toLowerCase().includes('abort');
  return {
    name:safeText(existing?.name??source.name)??'MediaClientError',
    stage:existing?.stage??fallbackStage,
    code:existing?.code??safeText(source.code)??(abort?'aborted':'media_operation_failed'),
    message,
...
async function normalizeFunctionInvokeError(error:unknown,dataError?:unknown):Promise<unknown> {
  ...
  return {
    name:source.name,message:responseCode??source.message,code:responseCode??source.code,
    details:body.details??source.details,hint:body.hint??source.hint,
    status:source.context?.status??source.status,
  };
}
export function shouldNormalizeImageForR2(mimeType:string|undefined,uri:string,fileName?:string):boolean {
  ...
  if(mime.startsWith('audio/')||mime.startsWith('video/')||mime==='application/pdf') return false;
  return mime==='image/heic'||mime==='image/heif'||mime==='application/octet-stream'
    ||mime===''||mime.startsWith('image/');
}
export async function normalizeImageForR2Upload(
  input:Pick<UploadMediaInput,'uri'|'mimeType'|'fileName'|'sizeBytes'|'purpose'>,
  ...
  if(!shouldNormalizeImageForR2(input.mimeType,input.uri,input.fileName)) return {...input};
  const operationId=createMediaOperationId('normalize');
  try {
    const result=await convert(input.uri,[],{compress:0.9,format:SaveFormat.JPEG});
    const convertedFile=new File(result.uri);
    const base=(input.fileName||'image').replace(/\.[^.]+$/,'');
    return {uri:result.uri,mimeType:'image/jpeg',fileName:`${base}.jpg`,sizeBytes:convertedFile.size};
  } catch {
    throw new MediaClientError({
      stage:'MEDIA_NORMALIZE_IMAGE',code:'image_normalization_failed',
      message:'image_normalization_failed',mimeType:input.mimeType,operationId,
    });
  }
}
export function extractRpcUuid(data:unknown,functionName:string):string {
  ...
  if(Array.isArray(candidate)) {
    if(candidate.length!==1) throw new MediaEntityRpcError({
      stage:'CAROUSEL_CLIENT_RESPONSE',code:'invalid_rpc_result',
      message:`invalid ${functionName} response array`,operationId:createMediaOperationId('rpc'),
    });
  ...
  if(typeof candidate!=='string'||!UUID_PATTERN.test(candidate)) {
    throw new MediaEntityRpcError({
      stage:'CAROUSEL_CLIENT_RESPONSE',code:'invalid_rpc_result',
      message:`invalid ${functionName} response type`,operationId:createMediaOperationId('rpc'),
    });
  }
  return candidate;
}
export function isRetryableAuthRpcError(error:unknown):boolean {
  ...
  return source.code==='42501'||text.includes('unauthorized')||text.includes('invalid jwt')
    ||text.includes('jwt expired')||numericStatus(source.status)===401;
}
...
export function validateCommonLinkedEntityRows(
  assetIds:string[],
  links:{asset_id:unknown;entity_id:unknown;position:unknown;slot:unknown}[],
):string|null {
  if(assetIds.length<2||new Set(assetIds).size!==assetIds.length||links.length!==assetIds.length) return null;
  const requested=new Set(assetIds);
  if(links.some(link=>typeof link.asset_id!=='string'||!requested.has(link.asset_id)||link.slot!=='media')) return null;
  const entityIds=new Set(links.map(link=>link.entity_id).filter((value):value is string=>typeof value==='string'));
  if(entityIds.size!==1) return null;
  const positions=links.map(link=>Number(link.position)).sort((a,b)=>a-b);
...
export async function createMediaUpload(input:UploadMediaInput,operationId=createMediaOperationId()) {
  try {
    ...
    if(error||!data?.success||!data.data) {
      const responseError=await normalizeFunctionInvokeError(error,data?.error);
      throw asMediaClientError(responseError,'MEDIA_CREATE_UPLOAD_RECORD',{mimeType:input.mimeType,operationId});
    }
    return {file,contract:data.data};
  } catch(error) {
    throw asMediaClientError(error,'MEDIA_CREATE_UPLOAD_RECORD',{mimeType:input.mimeType,operationId});
  }
}
export async function finalizeMediaUpload(assetId:string,operationId=createMediaOperationId()):Promise<MediaAssetDescriptor> {
  try {
    const {data,error}=await supabase.functions.invoke('finalize-media-upload',{body:{asset_id:assetId}});
    if(error||!data?.success||!data.data) throw await normalizeFunctionInvokeError(error,data?.error);
    if(data.data.url&&rejectLocalUrl(data.data.url)) throw {message:'invalid_persisted_media_url',code:'invalid_persisted_media_url'};
...
export async function uploadMediaFromUri(input:UploadMediaInput):Promise<MediaAssetDescriptor> {
  ...
  if(!input.uri) throw new MediaClientError({
    stage:'MEDIA_INPUT',code:'invalid_media_input',message:'invalid_media_input',
    mimeType:input.mimeType,operationId,
  });
...
export async function deleteMediaAsset(assetId:string):Promise<void> {
  ...
  try {
    ...
    if(error||!data?.success) throw await normalizeFunctionInvokeError(error,data?.error);
  } catch(error) {
...
export async function findCommonLinkedEntityForAssets(
  ...
  if(assetIds.length<2||new Set(assetIds).size!==assetIds.length) return null;
  const {data:sessionData}=await supabase.auth.getSession();
  const userId=sessionData.session?.user?.id;
  if(!userId) return null;
  const {data:links,error}=await supabase.from('media_asset_links')
    .select('asset_id,entity_id,position,slot')
    .in('asset_id',assetIds).eq('entity_type',entityType).eq('slot','media');
  if(error||!links) return null;
  const entityId=validateCommonLinkedEntityRows(assetIds,links);
  if(!entityId) return null;
  const {data:entity,error:entityError}=await supabase.from('videos')
    .select('id').eq('id',entityId).eq('user_id',userId).maybeSingle();
