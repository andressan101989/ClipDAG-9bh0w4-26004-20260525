...
export const DEEPAR_FILTERS: DeepARFilter[] = [
  // ── Face (a–z by id) ────────────────────────────────────────────────────
  // NOTE: localModule only set for files confirmed to exist in assets/deepar/.
  // All others use remote CDN download (cached after first use).
  ...
  { id: 'pixel_hearts',          name: 'Pixel Hearts',   emoji: '❤️', category: 'social',
    localModule: require('../assets/deepar/Pixel_Hearts.deepar'),
    remoteUrl: `${DEEPAR_CDN}Pixel_Hearts`,             description: 'Corazones pixelados animados' },
...
export async function prefetchDeepARFilters(filterIds?: string[]): Promise<void> {
  ...
  const ids = filterIds ?? ['flower_crown', 'lion', 'beauty', 'fire', 'fairy_lights'];
  await Promise.all(
    DEEPAR_FILTERS
