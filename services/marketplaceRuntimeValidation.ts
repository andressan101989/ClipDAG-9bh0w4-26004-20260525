...
const fail = (path: string): never => {
  throw new MarketplacePayloadError(path);
};
export const rpcObject = (
  value: unknown,
  path = "payload",
...
export const rpcText = (value: unknown, path: string): string =>
  typeof value === "string" && value !== "undefined" && value !== "null"
    ? value
    : fail(path);
export const rpcNonEmptyString = (value: unknown, path: string): string =>
  typeof value === "string" &&
  value.length > 0 &&
  value !== "undefined" &&
  value !== "null"
    ? value
    : fail(path);
export const rpcString = rpcNonEmptyString;
export const rpcNullableText = (value: unknown, path: string): string | null =>
  value === null ? null : rpcText(value, path);
export const rpcNullableString = (
  value: unknown,
  path: string,
): string | null => (value === null ? null : rpcNonEmptyString(value, path));
const UUID =
...
export const rpcStringArray = (value: unknown, path: string): string[] =>
  rpcArray(value, path).map((entry, index) =>
    rpcString(entry, `${path}[${index}]`),
  );
...
export const rpcCursorPage = (value: unknown, path = "page"): RpcCursorPage => {
  ...
  const nextCursor =
    row.next_cursor === null
      ? null
      : rpcObject(row.next_cursor, `${path}.next_cursor`);
