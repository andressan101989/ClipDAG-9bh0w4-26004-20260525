...
export async function createMarketplacePromotion(input:{productId:string;variantId?:string|null;type:MarketplacePromotionType;value:number;startsAt:Date;endsAt:Date}){const{data,error}=await db().rpc("create_marketplace_product_promotion",{p_product_id:input.productId,p_variant_id:input.variantId??null,p_promotion_type:input.type,p_value:input.value,p_starts_at:input.startsAt.toISOString(),p_ends_at:input.endsAt.toISOString(),p_idempotency_key:randomUUID()});if(error)throw error;return data;}
