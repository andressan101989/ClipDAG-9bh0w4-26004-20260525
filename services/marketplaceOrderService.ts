...
export interface CreateCheckoutReservationResult {checkout:MarketplaceCheckoutSummary;shippingAddress:{recipientName:string;city:string;region:string;country:string};orders:MarketplaceOrderSummary[]}
...
function mapResponse(value:unknown):CreateCheckoutReservationResult {
  ...
  return {checkout:{id:String(c.id),reference:String(c.reference),status:String(c.status) as MarketplaceCheckoutStatus,currency:'BDAG',subtotal:numberValue(c.subtotal),total:numberValue(c.total),expiresAt:String(c.expires_at),createdAt:String(c.created_at)},shippingAddress:{recipientName:String(a.recipient_name),city:String(a.city),region:String(a.region),country:String(a.country)},orders};
}
