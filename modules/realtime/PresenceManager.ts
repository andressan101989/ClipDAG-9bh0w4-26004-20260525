...
export type PresenceStatus = OnlineStatus | 'in_battle';
...
class PresenceManagerImpl {
  ...
  private _status:       PresenceStatus = 'offline';
  ...
  private _directHandlers= new Map<string, Set<(presence: PresenceData) => void>>();
  ...
  get currentStatus(): PresenceStatus       { return this._status; }
  get currentActivity(): ActivityType | null{ return this._activity; }

  async setStatus(status: PresenceStatus, _metadata?: unknown): Promise<void> {
  ...
  subscribe(userId: string, handler: (presence: PresenceData) => void): () => void {
    let handlers = this._directHandlers.get(userId);
    if (!handlers) { handlers = new Set(); this._directHandlers.set(userId, handlers); }
    handlers.add(handler);
    this.watchUsers([userId]);
    handler(this.getPresence(userId) ?? this._offline(userId));
    return () => {
      const current = this._directHandlers.get(userId);
      current?.delete(handler);
      if (!current?.size) { this._directHandlers.delete(userId); this.unwatchUsers([userId]); }
    };
  }
    ...
    try {
      ...
      for (const row of data) {
        ...
        const effectiveStatus: PresenceStatus =
          now - updatedAt > 120_000 ? 'offline' : (row.status as PresenceStatus) ?? 'offline';
        ...
        if (!prev || prev.status !== next.status || prev.activity !== next.activity) {
          ...
          this._notifyDirect(row.user_id, next);
        }
      }
  ...
  private _notifyDirect(userId: string, presence: PresenceData): void {
    for (const handler of this._directHandlers.get(userId) ?? []) {
      try { handler(presence); } catch { /* isolate */ }
    }
  }

  private _offline(userId: string): PresenceData {
    return { userId, status: 'offline', activity: null, sessionType: null, updatedAt: 0 };
  }
}
