...
try {
  ...
  assert(
    "own_pin",
    ownPin.commerce_mode === "own_product" &&
      ownPin.creator_commission_bps === 0,
  );
  assert(
    "affiliate_pin",
  ...
  const precedenceCandidates = await fetchAllCandidates(
      ids.session,
      hostB.token,
    ),
  ...
  const lowerSpecificResolution = await resolveAffiliateOffer(
    ...
    higherPublicFallback = await resolveAffiliateOffer(
      ids.productC,
      creatorD.id,
    );
  assert(
  ...
  assert(
    ...
    revocationShelf.find((item) => item.id === removedPin.id)?.availability ===
      "affiliate_offer_unavailable" &&
      revocationShelf.find((item) => item.id === expiredPin.id)
  ...
  assert(
    ...
    replacedCandidate?.is_pinned === true &&
      replacedCandidate?.candidate_availability ===
  ...
  const finalPaymentReconciliation = await rpc(
