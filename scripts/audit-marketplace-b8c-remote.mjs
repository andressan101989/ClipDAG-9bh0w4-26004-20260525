...
try {
 ...
 case when to_regprocedure('public.marketplace_admin_health_failure_count(text,jsonb)')is null then false else position('confirmed_state_breakdown' in pg_get_functiondef(to_regprocedure('public.marketplace_admin_health_failure_count(text,jsonb)')))>0 and position('escrow_expected_held_total' in pg_get_functiondef(to_regprocedure('public.marketplace_admin_health_failure_count(text,jsonb)')))>0 and position('refunded_settlement_breakdown' in pg_get_functiondef(to_regprocedure('public.marketplace_admin_health_failure_count(text,jsonb)')))>0 end health_classifier_aligned,
