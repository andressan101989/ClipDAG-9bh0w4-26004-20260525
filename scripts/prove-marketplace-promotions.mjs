...
const finalCorrective=readFileSync(new URL("../supabase/migrations/20260809210000_finalize_marketplace_promotion_idempotency.sql",import.meta.url),"utf8");
const timer=readFileSync(new URL("../services/marketplacePromotionTimer.ts",import.meta.url),"utf8");
...
assert.match(finalCorrective,/prior\.product_id is distinct from p_product_id/);assert.match(finalCorrective,/prior\.variant_id is distinct from p_variant_id/);assert.match(finalCorrective,/prior\.promotion_type is distinct from p_promotion_type/);assert.match(finalCorrective,/prior\.starts_at is distinct from p_starts_at/);assert.match(finalCorrective,/prior\.ends_at is distinct from p_ends_at/);
assert.match(finalCorrective,/round\(p_value,2\)/);assert.match(finalCorrective,/round\(p_value,8\)/);assert.match(finalCorrective,/marketplace_promotion_idempotency_conflict/);
assert.match(timer,/MAX_PROMOTION_TIMER_MS=2_147_000_000/);assert.match(timer,/Math\.min\(MAX_PROMOTION_TIMER_MS,Math\.max\(50/);
...
console.log(JSON.stringify({ok:true,fixtures:{percent:80,fixed:85,promotionalPrice:72.5,variantA:90,variantB:108,variantSpecificB:100,invalidFixedFallback:25,invalidPromotionalPriceFallback:70,validPercentageAfterEdit:72},interval:"[start,end)",precedence:"variant_over_product",singleResolutionSnapshot:true,endRetryIdempotent:true,createRetry:{samePayloadSameRow:true,changedPayloadConflict:true,rowCount:1},timer:{normalDelay:true,longDelayCapped:true},paymentReconciliation:0,settlementReconciliation:0,commissionReconciliation:0}));
