...
// Evidence references are repository paths and line numbers, never user records or secrets.
...
export const canonicalDrafts: readonly CanonicalDraft[] = [
  { document: privacy, provenance: [
    ...
    p('legal-choices-contact', 'legal_review_required', ['contexts/AuthContext.tsx:286', 'app/login.tsx:48'], 'Owner selected account age 13 and creator-exclusive age 18 in NPW-F-C5-B2-C1. Incorporation, age enforcement activation, contacts, legal review and dates remain open.'),
  ] },
  { document: terms, provenance: [
    p('accounts-eligibility', 'legal_review_required', ['contexts/AuthContext.tsx:286', 'contexts/AuthContext.tsx:302', 'app/login.tsx:48'], 'Owner selected account age 13 and creator-exclusive age 18. DOB collection and private classification are implemented; the production signup hook and creator-exclusive access boundary remain gated.'),
    ...
    p('legal-terms', 'legal_review_required', ['app/terms-of-service.tsx:193'], 'Florida and Miami-Dade owner policy draft, no mandatory arbitration; legal review, entity verification, liability, notice and effective date open.'),
  ] },
];
...
export const legacyReconciliation = [
  ...
  legacy('age 13+', '2025 and 2026 mobile text', 'REWRITE_FACTUAL', 'The owner now selects 13+ accounts and 18+ creator-exclusive access; protected legacy routes still need legal review before activation', ['app/terms-of-service.tsx:73', 'app/legal.tsx:39']),
  ...
  legacy('seven-day payment', '2025 Monetization hub', 'REMOVE_UNSUPPORTED', 'Settlement has conditional policy and dispute holds; no universal deadline', ['app/legal.tsx:151', 'supabase/migrations/20260804100000_marketplace_automatic_settlement_policy.sql:70']),
  legacy('85/15 gifts', '2025 Monetization hub', 'REMOVE_UNSUPPORTED', 'Gift authority must be assessed independently; split not adopted', ['app/legal.tsx:147', 'supabase/migrations/20260711090000_live_gifts_use_bdag_wallet.sql:1']),
  legacy('2FA', '2025 Privacy hub', 'REMOVE_UNSUPPORTED', 'No app-enforced 2FA established in audited auth path', ['app/legal.tsx:77', 'contexts/AuthContext.tsx:286']),
  legacy('24/7 monitoring', '2025 Privacy hub', 'REMOVE_UNSUPPORTED', 'No operational coverage guarantee established', ['app/legal.tsx:77']),
  legacy('regular audits', '2025 Privacy hub', 'REMOVE_UNSUPPORTED', 'No schedule or attestation established', ['app/legal.tsx:77']),
  legacy('30-day notice', '2025 Terms hub', 'OWNER_DECISION', 'No changes-to-terms notice rule selected', ['app/legal.tsx:35']),
  legacy('immediate changes', '2026 standalone Terms', 'OWNER_DECISION', 'No changes-to-terms notice rule selected', ['app/terms-of-service.tsx:185']),
  legacy('arbitration', '2026 standalone Terms', 'REWRITE_FACTUAL', 'Owner proposes no mandatory arbitration or waivers; legal review pending', ['app/terms-of-service.tsx:193']),
  legacy('jurisdiction', '2026 standalone Terms', 'REWRITE_FACTUAL', 'Owner proposes Florida law and competent Miami-Dade forum subject to mandatory rights; legal review pending', ['app/terms-of-service.tsx:193']),
] as const;
