...
describe("SUPERUSER-UI-PRESENTATION-A2-A4",()=>{
  ...
  it("rejects non-HTTPS preview input",()=>{render(<AdminMediaPreview url="javascript:alert(1)" kind="image"/>);expect(screen.getByText("Sin vista previa")).toBeInTheDocument();expect(screen.queryByRole("img")).not.toBeInTheDocument()});
  ...
  it("contains a lazy scoped HLS implementation without autoplay",()=>{const source=readFileSync(join(process.cwd(),"src","components","AdminPresentation.tsx"),"utf8");expect(source).toContain('import("hls.js")');expect(source).toContain("application/vnd.apple.mpegurl");expect(source).toContain("hls.loadSource(url)");expect(source).not.toMatch(/<video[^>]*autoplay/i)});
});
