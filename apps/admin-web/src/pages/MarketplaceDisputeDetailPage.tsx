  ...
  return (
    <div className="detail">
      ...
      <div className="detail-grid dispute-detail-grid">
        ...
        {terminal ? <section className="panel dispute-section-wide"><h3>Decisión final</h3><p>Este expediente es de solo lectura.</p><p>{text(decision.outcome || dispute.status)}</p></section> : <OperationConfirm minReasonLength={2} maxReasonLength={100} title="Resolver disputa" actions={[{value:"manual_review",label:"Enviar a revisión",reasonRequired:true},{value:"refund_buyer",label:"Reembolsar al comprador",reasonRequired:true,danger:true},{value:"release_seller",label:"Resolver a favor del vendedor y liberar ahora",reasonRequired:true,danger:true},{value:"reject_claim",label:"Rechazar reclamo y mantener liberación normal",reasonRequired:true,danger:true}]} onRun={run} />}
        {pendingRejectRelease ? <section className="panel dispute-section-wide"><h3>Fondos pendientes</h3><p>El reclamo fue rechazado. Los fondos siguen retenidos y continuarán por el calendario normal de liquidación. También puedes liberarlos ahora al vendedor.</p><OperationConfirm minReasonLength={2} maxReasonLength={100} title="Liberar fondos pendientes" actions={[{value:"release_seller",label:"Liberar fondos pendientes al vendedor",reasonRequired:true,danger:true}]} onRun={run} /></section> : null}
      </div>
    </div>
  );
}
