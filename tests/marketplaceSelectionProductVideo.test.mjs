...
const checkout=readFileSync("app/checkout.tsx","utf8"),product=readFileSync("app/product/[id].tsx","utf8"),seller=readFileSync("app/seller/product/[id]/variants.tsx","utf8"),picker=readFileSync("services/marketplaceMediaPickerService.ts","utf8"),migration=readFileSync("supabase/migrations/20260808150000_expose_public_marketplace_product_media.sql","utf8");
...
test("public media projection is exact-product ready public and publication gated",()=>{
 ...
 assert.match(product,/VideoView/);assert.match(product,/nativeControls/);assert.match(product,/kind === "video"/);assert.match(product,/Video/);
});
test("buyer options adapt and seller standard values are selection driven",()=>{
 assert.match(product,/option.values.length > 6/);assert.match(product,/reconcileVariantSelection/);assert.match(product,/isOptionValueSelectable/);
