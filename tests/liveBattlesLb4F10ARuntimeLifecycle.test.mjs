...
function stageHarness() {
  ...
  const {LiveBattleStage}=load(read('components/live/LiveBattleStage.tsx'),{'react':react,'react-native':rn,'@expo/vector-icons':{MaterialIcons:'Icon'},'@/constants/theme':theme,'@/components/live/LiveBattleViewerHUD':{LiveBattleViewerHUD:props=>element('HUD',props)},'@/services/liveBattleSpectatorService':spectator},{setInterval:fn=>{timers.add(fn);return fn;},clearInterval:fn=>timers.delete(fn)});
