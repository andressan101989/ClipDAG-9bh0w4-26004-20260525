...
test("same creator attribution merges while preserving the first opaque token", () => {
  ...
  const second = addMarketplaceCartItem(first.items, { ...attributed, attributionId: "attr-retry", quantity: 2 });
  assert.equal(second.result.ok, true);
  assert.equal(second.items[0].quantity, 3);
  assert.equal(second.items[0].attributionId, "attr-a");
});

test("conflicting creator attribution never silently overwrites an existing cart line", () => {
